...
from CFDP_Metric import CFDP

parser = argparse.ArgumentParser(description='Calculate energy-zone ratio')
...
parser.add_argument(
    ...
    type=str,
    default='score_conv/',
    help='Path to save channel scores')

parser.add_argument(
        '--alpha',
        type=float,
        default=0.03,
        help='Determine the contribution balance between Spatial & Frequency')
parser.add_argument(
        '--conv_fm',
        type=int,
        default=1,
        help='Select feature maps after conv or not')
...
parser.add_argument(
    '--pretrain_dir',
    type=str,
    default='./models/vgg_16_bn.pt',
    help='load the model from the specified checkpoint')
parser.add_argument(
    '--limit',
    type=int,
    default=5,
    help='The num of batch to get ratio.')
parser.add_argument(
    '--batch_size',
    type=int,
    default=128,
    help='Batch size for ratio generation, not training.')
parser.add_argument(
    '--gpu',
    type=str,
    default='0',
...
# Model
print(f'==> Building model using alpha = {args.alpha}')
...
if args.pretrain_dir:
    ...
    elif args.arch=='densenet_40' or args.arch=='resnet_110':
...
def get_feature_hook(self, input, output):
    '''Output is the feature map of the layer (B, C, H, W)
    '''
    ...
    for i in range(a):
        for j in range(b):
            CFDP(output[i,j,:,:], alpha=args.alpha).item()
    
    
    c = torch.tensor([CFDP(output[i,j,:,:], alpha=args.alpha).item() for i in range(a) for j in range(b)])
...
def get_feature_hook_densenet(self, input, output):
    ...
    c = torch.tensor([CFDP(output[i,j,:,:], alpha=args.alpha).item() for i in range(a) for j in range(b-12,b)])
...
if args.arch=='vgg_16_bn':
    ...
    for i, cov_id in enumerate(relucfg):
        ...
        if not os.path.isdir(args.score_path + args.arch+'_limit%d'%(args.limit)):
            os.makedirs(args.score_path+args.arch+'_limit%d'%(args.limit))
        np.save(args.score_path+args.arch+'_limit%d'%(args.limit)+'/score_conv' + str(i + 1) + '.npy', feature_result.numpy())
    ...
    if not os.path.isdir(args.score_path + args.arch+'_limit%d'%(args.limit)):
        os.makedirs(args.score_path + args.arch+'_limit%d'%(args.limit))
    np.save(args.score_path + args.arch+'_limit%d'%(args.limit)+ '/score_conv%d' % (1) + '.npy', feature_result.numpy())
    feature_result = torch.tensor(0.)
    total = torch.tensor(0.)
    ...
    for i in range(3):
        ...
        for j in range(9):
            if args.conv_fm == 1:
                conv_layer = block[j].conv1
            else:
                cov_layer = block[j].relu1
            handler = cov_layer.register_forward_hook(get_feature_hook)
            inference()
            handler.remove()
            np.save(args.score_path + args.arch +'_limit%d'%(args.limit)+ '/score_conv%d'%(cnt + 1)+'.npy', feature_result.numpy())
            cnt+=1
            feature_result = torch.tensor(0.)
            total = torch.tensor(0.)
            
            if args.conv_fm == 1:
                conv_layer = block[j].conv2
            else:
    ...
    if not os.path.isdir('score_conv_dense/' + args.arch+'_limit%d'%(args.limit)):
        os.makedirs('score_conv_dense/' + args.arch+'_limit%d'%(args.limit))
    ...
    for i in range(3):
        ...
        for j in range(12):
            ...
            np.save('score_conv_dense/' + args.arch +'_limit%d'%(args.limit) + '/score_conv%d'%(13*i+j+1)+'.npy', feature_result.numpy())
            feature_result = torch.tensor(0.)
            total = torch.tensor(0.)
        ...
        if i<2:
            ...
            if args.conv_fm == 1:
                conv_layer = trans.conv1
            else:
                conv_layer = trans.relu
    
            handler = cov_layer.register_forward_hook(get_feature_hook_densenet)
            inference()
            handler.remove()

            np.save('score_conv_dense/' + args.arch +'_limit%d'%(args.limit) + '/score_conv%d' % (13 * (i+1)) + '.npy', feature_result.numpy())
            feature_result = torch.tensor(0.)
            total = torch.tensor(0.)#
    if args.conv_fm == 1:
        cov_layer = net.conv1
    ...
    inference()
    handler.remove()
    np.save('score_conv_dense/' + args.arch +'_limit%d'%(args.limit) + '/score_conv%d' % (39) + '.npy', feature_result.numpy())
    feature_result = torch.tensor(0.)
    total = torch.tensor(0.)
    ...
    for idx, cov in enumerate(cov_list):
        ...
        if idx>0:
            for idx1,tp in enumerate(tp_list):
                if idx1==3:
                    np.save(args.score_path + args.arch+'_limit%d'%(args.limit) + '/score_conv%d_'%(idx+1)+tp+'.npy',
            ...
            np.save(args.score_path + args.arch+'_limit%d'%(args.limit) + '/score_conv%d' % (idx + 1) + '.npy',feature_result.numpy())
        feature_result = torch.tensor(0.)
        total = torch.tensor(0.)
    ...
    for i in range(3):
        ...
        for j in range(18):
            ...
            inference()
            handler.remove()
            np.save(args.score_path + args.arch  + '_limit%d' % (args.limit) + '/score_conv%d' % (
    ...
    if not os.path.isdir(args.score_path + args.arch+'_limit%d'%(args.limit)):
        os.mkdir(args.score_path + args.arch+'_limit%d'%(args.limit))
    np.save(args.score_path + args.arch+'_limit%d'%(args.limit) + '/score_conv%d' % (1) + '.npy', feature_result.numpy())
    feature_result = torch.tensor(0.)
    total = torch.tensor(0.)
    ...
    for i in range(4):
        ...
        for j in range(net.num_blocks[i]):
            ...
            inference()
            handler.remove()
            np.save(args.score_path + args.arch+'_limit%d'%(args.limit) + '/score_conv%d'%(cnt+1)+'.npy',
    ...
    if not os.path.isdir(args.score_path + args.arch + '_limit%d' % (args.limit)):
        os.mkdir(args.score_path + args.arch + '_limit%d' % (args.limit))
    np.save(args.score_path + args.arch + '_limit%d' % (args.limit) + '/score_conv%d' % (1) + '.npy',
            feature_result.numpy())
    feature_result = torch.tensor(0.)
    total = torch.tensor(0.)
