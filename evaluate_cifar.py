...
parser.add_argument(
    '--score_conv_prefix',
    type=str,
    default='./score_conv/vgg_16_bn_limit5',
    help='ratio conv file folder')

parser.add_argument(
    '--compress_rate',
    type=str,
    default='[0.21]*7+[0.75]*5',
...
def load_vgg_model(model, oristate_dict):
    ...
    cnt=0
    prefix = args.score_conv_prefix+'/score_conv'
    subfix = ".npy"
    for name, module in model.named_modules():
        ...
        if isinstance(module, nn.Conv2d):
            ...
            if orifilter_num != currentfilter_num:
                ...
                cov_id = cnt
                logger.info('loading ratio from: ' + prefix + str(cov_id) + subfix)
                ratio = np.load(prefix + str(cov_id) + subfix)
                select_index = np.argsort(ratio)[orifilter_num-currentfilter_num:]  # preserved filter id
                select_index.sort()
    ...
    for name, module in model.named_modules():
        ...
        if isinstance(module, Inception):
            ...
            for weight_index in honey_filter_index:
                ...
                if orifilter_num != currentfilter_num:
                    logger.info('loading ratio from: ' + prefix + str(cov_id) + branch_name + subfix)
                    ratio = np.load(prefix + str(cov_id)  + branch_name + subfix)
                    select_index = np.argsort(ratio)[orifilter_num - currentfilter_num:]  # preserved filter id
                    select_index.sort()
                else:
                    select_index = list(range(0, orifilter_num))
    ...
    for name, module in model.named_modules():
        ...
        if isinstance(module, nn.Conv2d):
            ...
            if orifilter_num != currentfilter_num:
                logger.info('loading ratio from: ' + prefix + str(cov_id) + subfix)
                ratio = np.load(prefix + str(cov_id) + subfix)
                select_index = list(np.argsort(ratio)[orifilter_num-currentfilter_num:])  # preserved filter id
                select_index.sort()
...
def main():
    ...
    while epoch < args.epochs:
        ...
        utils.save_checkpoint({
            ...
            }, is_best, args.job_dir, args.arch)
